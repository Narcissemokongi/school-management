...
export function BulletinEnfant({
  ...
  const notesRaw = useQuery(
    ...
    eleveId && anneeId && userId ? { eleveId, anneeId, userId } : "skip"
  );
  ...
  const periodes = useMemo(() => {
    ...
    return [...new Set(notes.map((n) => n.periode))]
      .sort()
      .slice(0, nbrPeriodes);
  }, [notes, nbrPeriodes]);
  ...
  const statsParMatiere = useMemo(() => {
    return matieres.map((matiere) => {
      ...
      const moyAnnuelleBrute = moyBrutes.every((m) => m !== "-")
        ? moyBrutes.reduce((sum, m) => sum + parseFloat(m), 0) /
          moyBrutes.length
        : null;

      const pourcentage =
        moyAnnuelleBrute !== null
          ? (moyAnnuelleBrute / baremeCours) * 100
          : null;
  ...
  if (moyenneGeneralePourcent !== "-" && seuils) {
    ...
    if (seuils.seuilFelicitations && moy >= seuils.seuilFelicitations)
      mention = "Félicitations";
    else if (seuils.seuilEncouragement && moy >= seuils.seuilEncouragement)
      mention = "Encouragement";
    else if (seuils.seuilAvertissement && moy <= seuils.seuilAvertissement)
      mention = "Avertissement";
  }

  const decision = eleve?.decisionConseil || "En attente";

  const absencesJustifiees = absences.filter(
    (a) => a.type === "absence" && a.statutJustification === "justifiee"
  ).length;
  const absencesNonJustifiees = absences.filter(
    (a) =>
      a.type === "absence" &&
      ...
      >
        <button
          ...
          style={{
            ...
            background: generating ? "#A5B4FC" : uiColors.accent,
            color: "white",
            border: "none",
            borderRadius: 10,
            fontWeight: 600,
      ...
      >
        ...
        <div
          style={{
            textAlign: "center",
            marginBottom: 12,
            borderBottom: `1px solid ${bulletinColors.accent}`,
            paddingBottom: 8,
          }}
        >
          <div
          ...
          >
            ...
            <div>
              ...
              >
                Enseignement{" "}
                {typePeriode === "trimestre" ? "Trimestriel" : "Semestriel"}
              </p>
            </div>
          </div>
          <h2
        ...
        <div
          style={{
            ...
            marginBottom: 10,
            padding: "8px 10px",
            background: bulletinColors.mutedBg,
            borderRadius: 6,
            gap: 4,
            fontSize: 11,
          }}
        >
          <div>
            <strong>Élève :</strong> {nom} {postnom}
          </div>
          <div>
        ...
        <div style={{ overflowX: "auto", marginBottom: 10 }}>
          ...
          >
            <thead>
              <tr
                style={{
                  background: bulletinColors.tableHeaderBg,
                  color: "white",
                }}
              >
                ...
                <th
                  style={{
                    padding: 4,
                    textAlign: "center",
                    background: bulletinColors.accent,
                    color: "white",
                  }}
                >
                  Moy. %
        ...
        <div style={{ marginBottom: 10 }}>
          ...
          >
            ...
            >
              <span
                style={{ color: bulletinColors.textPrimary, fontSize: 10 }}
              >
                Absences justifiées
              </span>
              <span
        ...
        <div
          style={{
            marginBottom: 8,
            padding: "6px 10px",
            background: bulletinColors.decisionBg,
            borderRadius: 6,
            display: "flex",
            alignItems: "center",
            gap: 6,
