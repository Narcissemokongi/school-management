...
// ✅ FIX #2 — sorti du composant : référence stable pour useMemo
const SECTIONS_META = [
...
export function PolitiqueConfidentialite() {
  ...
  const filteredSections = useMemo(() => {
    if (!searchTerm.trim()) return SECTIONS_META;
    const q = searchTerm.toLowerCase();
    return SECTIONS_META.filter((s) => s.title.toLowerCase().includes(q));
  }, [searchTerm]);

  // ✅ FIX #4 — Set d'ids visibles pour filtrer aussi le contenu
  const visibleIds = useMemo(
    () => new Set(filteredSections.map((s) => s.id)),
    [filteredSections]
  );

  // ✅ FIX #3 — useCallback + dep stable
  ...
  const backToTopSize = 44;
  const backToTopIconSize = 20;
  const backToTopBottom = isMobile ? 16 : 24;
  const backToTopRight = isMobile ? 16 : 24;

  return (
    // ✅ FIX #6 — topRef supprimé (dead code)
    <div style={{ maxWidth: 900, margin: "0 auto", padding: containerPadding }}>
      ...
      {/* ✅ FIX #1 + #5 — keyframe préfixé + prefers-reduced-motion */}
      <style>{`
