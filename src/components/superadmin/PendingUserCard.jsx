...
// ✅ FIX #2 — Keyframes EXPORTÉS pour injection UNE SEULE FOIS par le parent.
// À importer + injecter dans PendingTab (ou tout autre parent).
export const PendingUserCardKeyframes = (
  <style>{`
    @keyframes puc-spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
...
export function PendingUserCard({
  ...
  // ✅ FIX #1 — hover via state au lieu de mutation DOM directe
  ...
  const handleReject = useCallback(async () => {
    ...
    try {
      ...
      // ✅ FIX #7 — ferme l'input UNIQUEMENT en cas de succès
  ...
  // ✅ FIX #6 — guard `user.nom`
  const fullName = [user?.nom, user?.postnom, user?.prenom]
    .filter(Boolean)
    .join(" ");
  const displayName = fullName || user?.nom || "Utilisateur";
  ...
  // ✅ FIX #1 — shadow via state (plus de mutation DOM)
  ...
  const cardStyle = {
    ...
    transition: noMotion
      ? "none"
      : "box-shadow 0.2s, background-color 0.3s, border-color 0.3s, opacity 0.3s, transform 0.3s",
    border: `1px solid ${
      selected
        ? dark
          ? "#818CF8"
  ...
  // ✅ FIX #3 — retirer role="button" + tabIndex à cause des boutons nested.
  // Le bouton chevron dédié gère l'accessibilité clavier.
  ...
  return (
    ...
    >
      ...
      {onToggleSelect && (
        ...
        >
          {selected ? (
            <CheckSquare size={checkboxSize} />
          ) : (
            <Square size={checkboxSize} />
          )}
        </button>
      )}
